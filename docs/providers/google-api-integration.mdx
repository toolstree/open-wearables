...
<Note>
  `google_health` is this provider's name everywhere — in payloads, in `?provider=` filters, and in every URL Open Wearables builds. Before the split it was reachable at `/google/`, and OAuth and webhook endpoints still accept that path so URLs already registered with Google keep resolving. Nothing publishes it any more.
</Note>
...
<Steps>
  <Step title="Enable the Health API and create an OAuth client">
    ...
    <Warning>
    **Upgrading from before the provider split?** Your client is registered under the old `/oauth/google/callback` path. Add the `google_health` URI above *before* deploying — Google accepts several authorized redirect URIs, so adding it ahead of time causes no downtime, and the old one keeps working until you remove it.
    </Warning>
