...
| `POST /api/v1/sdk/users/{user_id}/sync` | `sdk` | `apple`, `samsung`, `health_connect` | UUID4 batch ID |

<Note>
  The Android SDK sends `provider: "google"` for Health Connect; the endpoint normalizes it to `health_connect` before the payload is stored. Objects written before the provider split are still under the `google/` prefix.
</Note>
